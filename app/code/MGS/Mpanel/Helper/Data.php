		...
		if (is_readable($themeStyleFile)){
			...
			if(isset($parsedArray['class_setting']['product_background'])){
				foreach($parsedArray['class_setting']['product_background'] as $classAttribute=>$classString){
					$classAttribute = str_replace('_', '-', $classAttribute);
					$setting[$classString] = [$classAttribute=>$this->getStoreConfig('color/product_background/background_color', $storeId)];
				}
			}
		}
        $setting = array_filter($setting);
	...
	public function generateCssByStore($storeId){
	   ...
	   $custom_font = $this->getStoreConfig('mgstheme/fonts/custom_fonts_element', $storeId);
		...
		foreach ($fontStyle as $class => $style) {
         ...
         if(($this->getStoreConfig('color/product_background/background_color', $storeId) != '') && ($this->getStoreConfig('color/product_background/background_color', $storeId) != 'transparent')){
